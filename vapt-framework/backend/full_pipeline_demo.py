"""
Full VAPT Pipeline Demo: Recon -> Scanner -> Risk Scorer -> Report Generator
"""
import asyncio
import json
import time
...
async def run_full_pipeline(target: str = "example.com"):
    ...
    print("\n[Module 4] Report Generator...")
    report_gen = ReportOrchestrator(output_dir="reports")
    scan_context = report_gen.generate(scan_context)
    report_info = scan_context.get("report", {})
    print(f"  Report ID: {report_info.get('report_id')}")
    print(f"  File: {report_info.get('generated_file')}")
    ...
    # Save HTML report separately
    html_content = report_info.get("html_content", "")
    if html_content:
        with open("reports/vapt_report.html", "w", encoding="utf-8") as f:
            f.write(html_content)
        print("HTML report saved to reports/vapt_report.html")
